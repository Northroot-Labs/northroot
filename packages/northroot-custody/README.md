...
only includes objects in that project, and does not include objects whose
custody says they are `ephemeral`, `never-export`, `excluded`, or
`provider-rehydrated`; replica targets must use a replica destination role and
