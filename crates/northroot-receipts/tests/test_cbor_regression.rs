...
fn test_cbor_canonicalization_stable() {
    ...
    assert_eq!(canonical1, canonical2, "Canonical encoding not stable (1 vs 2)");
    assert_eq!(canonical2, canonical3, "Canonical encoding not stable (2 vs 3)");
}
...
fn test_json_adapter_preserves_semantics() {
    ...
    assert_eq!(receipt1, receipt2, "JSON adapter does not preserve semantics");
}
...
fn test_cbor_roundtrip_preserves_structure() {
    ...
    assert_eq!(receipt1, receipt2, "CBOR round-trip does not preserve structure");
}
...
fn test_all_vectors_canonicalize_correctly() {
    ...
    for path in &vectors {
        ...
        assert!(!canonical.is_empty(), "Canonical encoding should not be empty for {}", path);
