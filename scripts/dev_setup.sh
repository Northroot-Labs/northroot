...
# Compatibility for older Codex-only environment variables.
if [ -z "${NORTHROOT_DEEP_TOOLS+x}" ] && [ -n "${NORTHROOT_CODEX_DEEP_TOOLS+x}" ]; then
  export NORTHROOT_DEEP_TOOLS="$NORTHROOT_CODEX_DEEP_TOOLS"
fi
if [ -z "${NORTHROOT_INSTALL_HOOKS+x}" ] && [ -n "${NORTHROOT_CODEX_INSTALL_HOOKS+x}" ]; then
  export NORTHROOT_INSTALL_HOOKS="$NORTHROOT_CODEX_INSTALL_HOOKS"
fi

log() {
  printf '[northroot-dev-setup] %s\n' "$*"
}
...
ensure_rust_toolchain() {
  ...
  if [ "${NORTHROOT_DEEP_TOOLS:-0}" = "1" ]; then
...
install_git_hooks() {
  if [ "${NORTHROOT_INSTALL_HOOKS:-1}" != "1" ]; then
    return
  fi
...
report_environment() {
  ...
  if [ "${#missing[@]}" -gt 0 ]; then
    printf 'missing required Northroot development tools: %s\n' "${missing[*]}" >&2
    return 1
  fi
